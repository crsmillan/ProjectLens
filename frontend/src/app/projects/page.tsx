        ...
        <div className="pt-24 px-8 pb-12 max-w-7xl mx-auto">
          ...
          <section className="mb-8">
            ...
            <div className="flex items-center gap-6 border-b border-outline-variant/10 pb-4">
              {['All Projects', 'Active', 'Archived', 'Team Focus'].map((tab, i) => (
                <button 
                  key={tab} 
                  className={`text-sm font-bold transition-all ${
                    i === 0 ? 'text-on-surface border-b-2 border-primary pb-4 -mb-[18px]' : 'text-outline hover:text-on-surface'
                  }`}
                >
                  {tab}
                </button>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            {loading && projects.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                ...
                <button 
                  onClick={() => setIsModalOpen(true)}
                  className="bg-surface-lowest shadow-ambient rounded-2xl p-6 flex items-center justify-center gap-3 text-outline hover:text-primary hover:bg-primary/5 transition-all group border border-dashed border-outline-variant/20 h-20"
                >
              ...
              <div className="py-40 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-outline italic text-lg mb-8 max-w-md">No projects found.</p>
                <button 
                  onClick={() => setIsModalOpen(true)}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-on-surface/20 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-10 animate-in zoom-in-95 duration-300">
              <h3 className="text-3xl font-headline font-black mb-2 text-on-surface">New Project Curation</h3>
              <p className="text-outline mb-8">Define the architectural scope of your next masterpiece.</p>
              
              <form onSubmit={handleCreateProject} className="flex flex-col gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold text-outline tracking-widest">PROJECT NAME</label>
                  <input 
                    autoFocus
                    type="text" 
